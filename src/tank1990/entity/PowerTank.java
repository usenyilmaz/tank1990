...
public class PowerTank extends AbstractTank implements Runnable {
    String name = "PowerTank";
    int health = 2;
    int points = 300;

    private Random random = new Random();
    private long lastAIShotTime = 0;
    private long aiFireRate = 2500; // 2.5 seconds between shots (slower but more powerful)
    ...
    public PowerTank(int startX, int startY, Player player) {
        super(startX, startY);
        this.player = player;
        this.speed = 3; // Set the parent's speed field
        loadImages();
        loadExplosionImage();
        initializeThread();
    }
    ...
    // Static spawn method
    public static PowerTank spawn(Player player) {
        Random spawnRandom = new Random();
        int[] coordinates = SPAWN_COORDINATES[spawnRandom.nextInt(SPAWN_COORDINATES.length)];
        return new PowerTank(coordinates[0], coordinates[1], player);
    }

    private void initializeThread() {
        tankThread = new Thread(this, "PowerTank-" + System.currentTimeMillis());
        running = true;
        tankThread.start();
    }

    private void loadImages() {
        loadImageForDirection("UP", "powerup.png");
        loadImageForDirection("DOWN", "powerdown.png");
        loadImageForDirection("LEFT", "powerleft.png");
        loadImageForDirection("RIGHT", "powerright.png");
    }
    ...
    // AI shooting method - PowerTank shoots less frequently but more accurately
    public Bullet aiShoot() {
        ...
        // Random chance to shoot (50% chance - more accurate than other tanks)
    ...
    public void run() {
        while (running && !isExploding) {
            try {
                ...
                if (random.nextInt(100) < 3) { // 3% chance to change direction (reduced from 15%)
                ...
                Thread.sleep(18); // Slightly slower for PowerTank (55+ FPS)
