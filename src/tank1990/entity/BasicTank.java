...
public class BasicTank extends AbstractTank implements Runnable {
    String name = "BasicTank";
    int health = 1;
    int points = 100;

    private Random random = new Random();
    private long lastAIShotTime = 0;
    private long aiFireRate = 2000; // 2 seconds between shots
    ...
    public BasicTank(int startX, int startY, Player player) {
        super(startX, startY);
        this.player = player;
    ...
    // Static spawn method
    public static BasicTank spawn(Player player) {
        Random spawnRandom = new Random();
        int[] coordinates = SPAWN_COORDINATES[spawnRandom.nextInt(SPAWN_COORDINATES.length)];
        return new BasicTank(coordinates[0], coordinates[1], player);
    }

    private void initializeThread() {
        tankThread = new Thread(this, "BasicTank-" + System.currentTimeMillis());
        running = true;
        tankThread.start();
    }

    private void loadImages() {
        loadImageForDirection("UP", "basicup.png");
        loadImageForDirection("DOWN", "basicdown.png");
        loadImageForDirection("LEFT", "basicleft.png");
        loadImageForDirection("RIGHT", "basicright.png");
    }
    ...
    public Bullet aiShoot() {
        ...
        // Random chance to shoot (30% chance)
    ...
    public void run() {
        while (running && !isExploding) {
            try {
                ...
                if (random.nextInt(100) < 5) { // 5% chance to change direction (reduced from 20%)
                ...
                Thread.sleep(16); // Match game loop timing (60 FPS)
